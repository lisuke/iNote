...
class RegistrationForm(FlaskForm):
    email = StringField(_('Email'), validators=[DataRequired(), Length(1, 64),Email()])
    username = StringField(_('Username'), validators=[DataRequired(), Length(1, 64), Regexp('^[A-Za-z][A-Za-z0-9_.]*$', 0,_('Usernames must have only letters, numbers, dots or underscores'))])
    password = PasswordField(_('Password'), validators=[DataRequired(),Length(6,20,message=_('Passwords must >=6 and <=20.')),EqualTo('confirm', message=_('Passwords must match.'))])
